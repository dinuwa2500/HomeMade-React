...
const MyTickets = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        console.log("[MyTickets] Using token:", token);
        // Add timestamp to force fresh fetch and avoid 304
        const response = await axios.get(
          `http://localhost:8000/api/tickets/user`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        console.log("[MyTickets] Full backend response:", response);
        console.log("[MyTickets] Backend response data:", response.data);
        ...
        console.log("[MyTickets] Processed tickets:", updatedTickets);
        setTickets(updatedTickets);
        setResolvedCount(
          updatedTickets.filter((t) => t.status === "Resolved").length
        );
        setPendingCount(
          updatedTickets.filter((t) => t.status === "Pending").length
        );
      } catch (error) {
        setTickets([]);
  ...
  const updateTicket = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/api/tickets/update/${editTicketId}`,
        formData,
        {
          headers: {
  ...
  const openLightbox = (image) => {
    let imageUrl = image;
    if (image.match(/^\/?uploads\//)) {
      imageUrl = `http://localhost:8000/${image.replace(/^\/+/, "")}`;
    } else {
      imageUrl = `http://localhost:8000/uploads/${image}`;
    }
